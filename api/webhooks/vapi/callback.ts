...
const successfulEndings = ['customer-ended-call', 'assistant-ended-call', 'Sem Debito', 'Sem Débito'];
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!existingCall && !foundCall) {
      ...
      if (insertError) throw new Error(`Erro ao criar chamada: ${insertError.message}`);
