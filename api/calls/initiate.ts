...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { contactId, campaignId, customerNumber, customerName } = req.body as {
      contactId?: string; campaignId?: string; customerNumber?: string; customerName?: string;
    };

    if (!customerNumber || !customerName)
      return res.status(400).json({ success: false, error: 'Telefone e nome são obrigatórios' });
    ...
    if (campaignId) {
      const { data: campaign } = await supabase
        .from('campaigns').select('assistant_vapi_id, linha_vapi_id')
    ...
    const n8nPayload = {
      ...
      customerNumber, customerName, cpf,
      assistantId, phoneNumberId, callbackUrl, tipoTelefonia: 'vapi'
    };
    ...
    const response = await fetch(n8nWebhookUrl, {
      ...
      headers: {
        ...
        'x-source': 'node-backend', 'x-function-name': 'initiate-vapi-call',
        'x-system': 'discador-vapi', 'x-version': '1.0'
      },
